      ...
      begin
        ...
        repeat
          ...
          if cave_room_reachable(next, arrow) then
            { Arrow enters next room in path }
            arrow := next
          else
            begin
              writeLn();
...
procedure test ();
  var
    testCommand : playOption;
    testNewGame : gameOption;
    testRoom : caveRoom;
    testNumber : integer = 0;
    test_arrow_path : array[1..5] of integer = (5, 4, 3, 2, 1);
    test_flag : boolean;
  begin
    writeLn('Testing ...');
    ...
    { Test number input }
    repeat
      write('Enter a number, (99 to finish)');
      testNumber := input_number();
      writeLn('... ', testNumber);
    until testNumber = 99;
    { Test arrow path length input }
    arrow_path_length := 0;
    arrow_path := test_arrow_path;
    show_arrow_path();
    repeat
      prompt_arrow_path_length();
      show_arrow_path();
    until arrow_path_length = 5;
    { Test room number shuffling }
    shuffle_room_numbers();
    write('Shuffled room numbers: ');
    show_room_numbers();
    { Test cave }
    show_cave_rooms();
    write('2 reachable from 10...');
    writeLn(cave_room_reachable(2, 10));
    write('4 reachable from 16...');
    writeLn(cave_room_reachable(4, 16));
    { Test game state generation and description }
    new_game(NEW_ROOMS);
    show_game_state();
    describe_hunter_location();
    writeLn('Hunter in ', room_numbers[1], '?: ', cave_room_has_hunter(room_numbers[1]));
    writeLn('Hunter in ', room_numbers[7], '?: ', cave_room_has_hunter(room_numbers[7]));
    writeLn('Wumpus in ', room_numbers[2], '?: ', cave_room_has_wumpus(room_numbers[2]));
    writeLn('Wumpus in ', room_numbers[8], '?: ', cave_room_has_wumpus(room_numbers[8]));
    writeLn('Pit in ', room_numbers[3], '?: ', cave_room_has_pit(room_numbers[3]));
    writeLn('Pit in ', room_numbers[9], '?: ', cave_room_has_pit(room_numbers[9]));
    writeLn('Pit in ', room_numbers[4], '?: ', cave_room_has_pit(room_numbers[4]));
    writeLn('Pit in ', room_numbers[1], '?: ', cave_room_has_pit(room_numbers[1]));
    writeLn('Bats in ', room_numbers[5], '?: ', cave_room_has_bats(room_numbers[5]));
    writeLn('Bats in ', room_numbers[2], '?: ', cave_room_has_bats(room_numbers[2]));
    writeLn('Bats in ', room_numbers[6], '?: ', cave_room_has_bats(room_numbers[6]));
    writeLn('Bats in ', room_numbers[3], '?: ', cave_room_has_bats(room_numbers[3]));
    with current_game_state do
      begin
        warn_of_hazards(wumpus);
        warn_of_hazards(pits[1]);
        warn_of_hazards(pits[2]);
        warn_of_hazards(bats[1]);
        warn_of_hazards(bats[2]);
      end;
    { Test arrow path room input }
    repeat
      testNumber := prompt_arrow_path_room(1);
    until testNumber = 99;
    repeat
      testNumber := prompt_arrow_path_room(2);
    until testNumber = 99;
    repeat
      testNumber := prompt_arrow_path_room(3);
    until testNumber = 99;
    repeat
      testNumber := prompt_arrow_path_room(4);
    until testNumber = 99;
    repeat
      testNumber := prompt_arrow_path_room(5);
    until testNumber = 99;
    { Test arrow path entry }
    prompt_arrow_path();
    show_arrow_path();
    { Test command entry }
    repeat
      testCommand := prompt_command;
      case testCommand of
        MOVE_ME :
            writeLn('command: move Hunter');
        SHOOT :
            writeLn('command: shoot a crooked arrow');
        WHERE_AM_I :
            writeLn('command: describe Hunter location');
        GAME_STATE :
            writeLn('command: show current game state');
        CAVE_ROOMS :
            writeLn('command: describe the cave');
        otherwise
          writeLn('Huh?')
        end
    until testCommand = CAVE_ROOMS;
    { Test new game entry }
    repeat
      testNewGame := prompt_play_again();
      case testNewGame of
        SAME_ROOMS :
            writeLn('new game option: same rooms');
        NEW_ROOMS :
            writeLn('new game option: new rooms');
        QUIT_GAME :
            writeLn('new game option: quit');
        otherwise
          writeLn('Huh?')
        end
    until testNewGame = QUIT_GAME;
    { Test show game result }
    show_results(WON);
    show_results(LOST);
    show_results(IN_PLAY);
    { Test input room to move Hunter to }
    testRoom := prompt_new_hunter_room();
    writeLn('Hunter to move into room ', testRoom);
    { Test check if Wumpus ate Hunter }
    with current_game_state do
      begin
        { try with the Hunter in the same room as the Wumpus }
        hunter := wumpus;
        writeLn('Hunter in room with Wumpus? ', cave_room_has_wumpus(hunter));
        did_wumpus_eat_hunter();
        show_game_state();
        { reset and try with the Hunter in a different room }
        player := IN_PLAY;
        hunter := cave[hunter, 1];
        writeLn('Hunter in room with Wumpus? ', cave_room_has_wumpus(hunter));
        did_wumpus_eat_hunter();
        show_game_state()
      end;
    { Test move the Wumpus }
    with current_game_state do
      repeat
        writeLn('Hunter is in room: ', hunter);
        move_wumpus();
        writeLn('Wumpus is now in room: ', wumpus)
      until player = LOST;
    { Test check if arrow hit Wumpus }
    with current_game_state do
      begin
        { try with the arrow in the same room as the Wumpus }
        testRoom := wumpus;
        writeLn('Arrow is in room ', testRoom);
        writeLn('Arrow in room with Wumpus? ', cave_room_has_wumpus(testRoom));
        did_arrow_hit_wumpus(testRoom);
        show_game_state();
        { reset and try with the arrow in a different room }
        player := IN_PLAY;
        testRoom := cave[testRoom, 1];
        writeLn('Arrow is in room ', testRoom);
        writeLn('Arrow in room with Wumpus? ', cave_room_has_wumpus(testRoom));
        did_arrow_hit_wumpus(testRoom);
        show_game_state()
      end;
    { Test check if arrow hit Hunter }
    with current_game_state do
      begin
        { try with the arrow in the same room as the Hunter }
        testRoom := hunter;
        writeLn('Arrow is in room ', testRoom);
        writeLn('Arrow in room with Hunter? ', cave_room_has_hunter(testRoom));
        did_arrow_hit_hunter(testRoom);
        show_game_state();
        { reset and try with the arrow in a different room }
        player := IN_PLAY;
        testRoom := cave[testRoom, 1];
    ...
    { Test check if Hunter out of arrows }
    with current_game_state do
      begin
        { try with one arrow left }
        arrows := 1;
        writeLn('Arrows left ', arrows);
        are_arrows_all_gone();
        show_game_state();
        { reset and try with no arrows left }
        arrows := 0;
        writeLn('Arrows left ', arrows);
        are_arrows_all_gone();
        show_game_state()
    ...
    with current_game_state do
      begin
        { Begin with path set to rooms 5..1 }
        arrow_path_length := 5;
        arrow_path := test_arrow_path;
        wumpus := 20;
        hunter := 6;
        fly_arrow();
        { try with path containing Hunter's room }
        wumpus := 20;
        hunter := 4;
        fly_arrow();
        { try with path containing Wumpus's room }
        wumpus := 3;
        hunter := 6;
        fly_arrow()
      end;
    ...
    with current_game_state do
      begin
        writeLn('Test shoot an arrow');
        new_game(NEW_ROOMS);
        show_game_state();
        writeLn('Enter a path that won''t hit anything.');
        shoot_arrow();
        show_game_state();
        writeLn('---');
        new_game(SAME_ROOMS);
        show_game_state();
        writeLn('Enter a path that hits the Wumpus.');
        shoot_arrow();
        show_game_state();
        writeLn('---');
        new_game(SAME_ROOMS);
        show_game_state();
        writeLn('Enter a path that hits the Hunter.');
        shoot_arrow();
        show_game_state();
        writeLn('---');
        new_game(SAME_ROOMS);
        show_game_state();
        writeLn('Enter a path that goes astray after two rooms.');
        shoot_arrow();
        show_game_state();
        writeLn('---');
        new_game(SAME_ROOMS);
        show_game_state();
        writeLn('Enter a path that goes astray in the Hunter''s room.');
        shoot_arrow();
        show_game_state()
      end;
    { Test hunter entered room with bats }
    with current_game_state do
      begin
        writeLn('Hunter in a room with no bats.');
        new_game(NEW_ROOMS);
        test_flag := did_hunter_enter_room_with_bats();
        writeLn('Did Hunter enter a room with bats? ', test_flag);
        show_game_state();
        writeLn('Hunter in first room with bats.');
        hunter := bats[1];
        test_flag := did_hunter_enter_room_with_bats();
        writeLn('Did Hunter enter a room with bats? ', test_flag);
        show_game_state();
        writeLn('Hunter in second room with bats.');
        hunter := bats[2];
        test_flag := did_hunter_enter_room_with_bats();
        writeLn('Did Hunter enter a room with bats? ', test_flag);
        show_game_state()
      end;
    { Test hunter entered room with a pit }
    with current_game_state do
      begin
        writeLn('Hunter in a room with no pits.');
        new_game(NEW_ROOMS);
        did_hunter_enter_room_with_pit();
        show_game_state();
        writeLn('Hunter in first room with a pit.');
        hunter := pits[1];
        did_hunter_enter_room_with_pit();
        show_game_state();
        writeLn('Hunter in second room with a pit.');
        hunter := pits[2];
        did_hunter_enter_room_with_pit();
        show_game_state()
      end;
    { Test hunter entered room with the Wumpus }
    with current_game_state do
      begin
        writeLn('Hunter in a room with no Wumpus.');
        new_game(NEW_ROOMS);
        did_hunter_enter_room_with_wumpus();
        show_game_state();
        writeLn('Hunter in the room with the Wumpus.');
        hunter := wumpus;
        did_hunter_enter_room_with_wumpus();
        show_game_state();
      end;
